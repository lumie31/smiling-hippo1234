      ...
      <v-container class>
        <v-row>
          <v-col cols="12" sm="12" class="legalbox-wrapper d-flex align-center">
            <img src="@/assets/events.png" class="legalbox-icons" width="40" />
            <h2 class="display-1 ml-4">Events</h2>
          </v-col>
        </v-row>
        <v-row>
...
export default {
  data() {
    return {
      ...
      origParagraphLength: this.$store.state.Events.StaffContractParagraphs
        .length,
      paragraphList: [],
      clauseList: [],
      formOutput: {
  ...
  mounted() {
    this.paragraphList = this.$store.state.Events.StaffContractParagraphs;
  },
  methods: {
  ...
  computed: {
    fetchOwnBackgroundDetails() {
      let eachDoc = this.$store.state.EventsHeaders;
