      ...
      <v-container class>
        <v-row>
          <v-col cols="12" sm="12" class="legalbox-wrapper d-flex align-center">
            <img src="@/assets/fashion-and-beauty-icon.png" class="legalbox-icons" width="40" />
            <h2 class="display-1 ml-4">Fashion & Beauty</h2>
          </v-col>
        </v-row>
        <v-row>
...
export default {
  data() {
    return {
      ...
      origParagraphLength: this.$store.state.fashionAndBeauty
  ...
  mounted() {
    this.paragraphList = this.$store.state.fashionAndBeauty.partnershipContractParagraphs;
  },
  methods: {
  ...
  computed: {
    fetchOwnBackgroundDetails() {
      let eachDoc = this.$store.state.fashionAndBeautyHeaders;
