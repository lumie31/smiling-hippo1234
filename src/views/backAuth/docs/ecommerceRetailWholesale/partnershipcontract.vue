      ...
      <v-container class>
        <v-row>
          <v-col cols="12" sm="12" class="legalbox-wrapper d-flex align-center">
            <img src="@/assets/ecommerce.png" class="legalbox-icons" width="40" />
            <h2 class="display-1 ml-4">E-commerce, Retail & Wholesale</h2>
          </v-col>
        </v-row>
        <v-row>
...
export default {
  data() {
    return {
      ...
      origParagraphLength: this.$store.state.ecommerceRetailWholesale
  ...
  mounted() {
    this.paragraphList = this.$store.state.ecommerceRetailWholesale.partnershipContractParagraphs;
  },
  methods: {
  ...
  computed: {
    fetchOwnBackgroundDetails() {
      let eachDoc = this.$store.state.ecommerceRetailWholesaleHeaders;
